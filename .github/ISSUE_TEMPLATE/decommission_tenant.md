name: "Epic Sub-issue Automation"
...
jobs:
  generate-sub-issues:
    # Only execute if the newly created issue is labeled 'epic'
    ...
    steps:
      - name: Spawn Sub-Issues with Custom Descriptions
        ...
        run: |
          # ----------------------------------------------------
          # SUB-ISSUE 1: BACKEND DEVELOPMENT
          # ----------------------------------------------------
          gh issue create \
            --repo "$REPO" \
            --parent "$PARENT_ISSUE" \
            --title "⚙️ Backend: Database Schemas & API Architecture" \
            --label "backend,enhancement" \
            --body "### Technical Description
          This issue handles the core backend integration for the parent Epic #$PARENT_ISSUE.
